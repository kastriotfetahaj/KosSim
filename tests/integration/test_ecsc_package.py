...
SERVICES = {
    "svc1": ("svc1-ledgerforge", "/var/lib/ledgerforge", 3),
    "svc2": ("svc2-vaultgrid", "/var/lib/vaultgrid", 3),
    "svc3": ("svc3-specterlog", "/var/lib/specterlog", 3),
    "svc4": ("svc4-nanofleet", "/var/lib/nanofleet", 3),
    "svc5": ("svc5-policyforge", "/var/lib/policyforge", 3),
}


def test_service_metadata_and_checker_capabilities() -> None:
    common = (CHALLENGES / "common" / "ecsc_checker.py").read_text()
    for svc, (directory, _, flagstores) in SERVICES.items():
        ...
        assert "PUTFLAG" in checker and "GETFLAG" in checker
        if (base / "checker" / "src" / "checker.py").exists():
...
def test_persistence_paths_are_declared() -> None:
    ...
    for _, data_path, _ in SERVICES.values():
        assert data_path in compose
    for directory, data_path, _ in SERVICES.values():
...
def test_unique_interaction_markers_present() -> None:
    ...
    svc2_src = "\n".join(p.read_text() for p in (CHALLENGES / "svc2-vaultgrid" / "service" / "src").glob("*.cpp"))
    assert "/api/feed" in svc2_src and "/api/crypt" in svc2_src
