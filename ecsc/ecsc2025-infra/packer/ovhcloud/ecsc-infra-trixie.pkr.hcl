...
source "openstack" "ecsc-infra-trixie" {
  flavor                  = "d2-8"
  image_name              = "ecsc-infra-trixie-${local.timestamp}"
  image_tags              = [ "ecsc-infra-trixie" ]
  networks                = ["${var.ext_net_id}"]
  source_image_name       = "Debian 13 - UEFI"
  ssh_ip_version          = "4"
  ssh_username            = "debian"
...
build {
  sources = ["source.openstack.ecsc-infra-trixie"]
