...
source "openstack" "ecsc-router" {
  flavor                  = "d2-8"
  image_name              = "ecsc-router-${local.timestamp}"
  image_tags              = [ "ecsc-router" ]
...
build {
  sources = ["source.openstack.ecsc-router"]
  ...
  provisioner "ansible" {
    ...
    playbook_file    = "ansible/ecsc-router.yml"
    user             = "debian"
  }
}
