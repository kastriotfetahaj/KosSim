...
source "openstack" "ecsc-gameserver" {
  flavor                  = "d2-8"
  image_name              = "ecsc-gameserver-${local.timestamp}"
  image_tags              = [ "ecsc-gameserver" ]
...
build {
  sources = ["source.openstack.ecsc-gameserver"]
  ...
  provisioner "ansible" {
    ...
    host_alias       = "packer-gameserver"
    playbook_file    = "ansible/ecsc-gameserver.yml"
    user             = "debian"
  }
