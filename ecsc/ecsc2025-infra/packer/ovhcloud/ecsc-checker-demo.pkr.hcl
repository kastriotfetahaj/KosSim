...
source "openstack" "ecsc-checker-demo" {
  flavor                  = "d2-8"
  image_name              = "ecsc-checker-demo-${local.timestamp}"
  image_tags              = [ "ecsc-checker-demo" ]
...
build {
  sources = ["source.openstack.ecsc-checker-demo"]
  ...
  provisioner "ansible" {
    ...
    host_alias       = "packer-checker-demo"
    playbook_file    = "ansible/ecsc-checker-demo.yml"
    user             = "debian"
  }
