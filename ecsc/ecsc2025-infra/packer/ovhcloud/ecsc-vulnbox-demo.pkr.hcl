...
source "openstack" "ecsc-vulnbox" {
  flavor                  = "d2-8"
  image_name              = "ecsc-vulnbox-demo-${local.timestamp}"
  image_tags              = [ "ecsc-vulnbox-demo" ]
...
build {
  sources = ["source.openstack.ecsc-vulnbox"]
  ...
  provisioner "ansible" {
    ...
    host_alias       = "packer-vulnbox-demo"
    playbook_file    = "ansible/ecsc-vulnbox-demo.yml"
    user             = "ubuntu"
  }
}
