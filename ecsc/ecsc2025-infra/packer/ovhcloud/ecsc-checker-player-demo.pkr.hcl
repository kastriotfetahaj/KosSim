...
source "openstack" "ecsc-checker-player-demo" {
  flavor                  = "d2-8"
  image_name              = "ecsc-checker-player-demo-${local.timestamp}"
  image_tags              = [ "ecsc-checker-player-demo" ]
...
build {
  sources = ["source.openstack.ecsc-checker-player-demo"]
  ...
  provisioner "ansible" {
    ...
    host_alias       = "packer-checker-player-demo"
    playbook_file    = "ansible/ecsc-checker-player-demo.yml"
    user             = "debian"
  }
