...
source "openstack" "ecsc-checker-cte" {
  flavor                  = "d2-8"
  image_name              = "ecsc-checker-cte-${local.timestamp}"
  image_tags              = [ "ecsc-checker-cte" ]
...
build {
  sources = ["source.openstack.ecsc-checker-cte"]
  ...
  provisioner "ansible" {
    ...
    host_alias       = "packer-checker-cte"
    playbook_file    = "ansible/ecsc-checker-cte.yml"
    user             = "debian"
  }
