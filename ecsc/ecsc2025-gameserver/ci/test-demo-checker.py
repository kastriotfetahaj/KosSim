...
def check_log_messages(logs: t.List[t.Dict[str, t.Any]]) -> None:
    for log in logs:
        print(
            f"- #{log['id']:02d} {log['level']:2d} [{log['component']}] {log['title']} {log['text']}"
        )
    ...
    assert_title_in_logs(
        "Network open within teams only"
    )  # after the game teams should retain access to their vulnbox
...
def main() -> None:
    ...
    subprocess.run(
        ["python3", "./scripts/reset_ctf.py"], input=b"y", check=True, timeout=20
    )
    handles = []
    ...
    try:
        ...
        # Recreate scoreboard
        subprocess.run(
            ["python3", "./scripts/recreate_scoreboard.py"], check=True, timeout=15
        )
        ...
        count = len(re.findall(rb"\] stderr-Test", output))
        if TICK_COUNT != count:
            raise Exception(
