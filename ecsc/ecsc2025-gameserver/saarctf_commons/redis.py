...
REDIS_LOGGER = logging.getLogger("redislogger")
...
def get_redis_connection() -> redis.StrictRedis:
    ...
    if not redis_default_connection_pool:
        REDIS_LOGGER.warning("Creating new connection-pool instance")
        redis_default_connection_pool = redis.ConnectionPool(
            connection_class=NamedRedisConnection, **config.REDIS
        )
