...
def submit_random_flags(endround: int, chance=0.33):
    ...
    for s in services:
        ...
        for t in teams:
            team_exploit_ready[(t.id, s.id)] = (
                random.randint(1, round(endround / chance)) * service_difficulty
            )
            team_patch_ready[(t.id, s.id)] = (
                random.randint(1, round(endround)) * service_difficulty // 3
            )
            if t.name == "saarsec":
                team_exploit_ready[(t.id, s.id)] = int(
                    team_exploit_ready[(t.id, s.id)] * 0.8
                )
                team_patch_ready[(t.id, s.id)] = int(
                    team_patch_ready[(t.id, s.id)] * 0.8
                )
    for s in services:
        submitted_flags = []
        sid = s.id
        for attacker in teams:
            ...
            for victim in teams:
                for r in range(
                    ...
                    p = 0
                    if s.num_payloads > 1:
                        p = random.randint(0, s.num_payloads - 1)
                    flag = SubmittedFlag(
