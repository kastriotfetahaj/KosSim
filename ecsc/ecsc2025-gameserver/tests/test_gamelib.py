...
from gamelib import gamelib, Team, ServiceConfig
        ...
        super().__init__(
            ServiceConfig(
                ...
                flag_ids=[
                    ...
                    "username",
                    "email",
                    "pattern:${username}/abc/${alphanum12}",
                ],
                interface_class="",
                interface_file="",
            )
        ...
        for team in TEST_TEAMS:
            for tick in (1, 2, 0, -1, 1338):
                for payload in (0, 1, 1339):
                    ...
                    seen_flags.add(flag)
                    self.assertTrue(
                        gamelib.get_flag_regex().fullmatch(flag),
                        f"Flag {flag} did not match regex {gamelib.get_flag_regex()}",
                    )
                    # parse flag
            ...
            for team in TEST_TEAMS:
                for round in (1, 2, 0, -1, 1338):
                    for index in range(len(service.config.flag_ids)):
                        ...
                        self.assertNotIn(
                            flag_id, seen_flag_ids, f"Flag ID {flag_id} repeated"
                        )
                        ...
                        for _ in range(3):
                            ...
                            self.assertEqual(
                                flag_id,
                                flag_id_2,
                                f"Flag ID not deterministic: {flag_id} != {flag_id_2}",
                            )
        self.assertListEqual(
            flag_ids[0], flag_ids[1], "Multiple runs give different flag ids"
        )
