...
class ScriptRunner:
    @classmethod
    def run_script(
        cls, file: str, args: list[str], check: bool = True, timeout: int = 10
        ...
        with TemporaryDirectory() as config_dir:
            ...
            env["CONFIG_FILE"] = os.path.join(config_dir, "config.json")
    ...
    def run_script_for_time(
        ...
        with TemporaryDirectory() as config_dir:
            ...
            with subprocess.Popen(
                cmd, cwd=cwd, env=env, stderr=subprocess.PIPE, stdout=subprocess.PIPE
            ) as process:
                try:
            ...
            if check and result.returncode != 0:
                ...
                raise Exception(
                    f"Process terminated with return code {result.returncode}"
                )
            return result
    ...
    def parse_influx_format(cls, data: str) -> dict[str, list[dict[str, int | str]]]:
        ...
        for line in data.split("\n"):
            ...
            if parts1[0] not in result:
                result[parts1[0]] = []
            result[parts1[0]].append(
                {"time": int(parts[-1])}
                | dict(x.split("=", 1) for x in parts1[1:] + parts2)
            )
        return result
