...
class BenchTestCase(DatabaseTestCase):
    ...
    def _print_stats(self):
        ...
        flags_per_service = (
            SubmittedFlag.query.with_entities(SubmittedFlag.service_id, func.count())
            .group_by(SubmittedFlag.service_id)
            .order_by(SubmittedFlag.service_id)
            .all()
        )
        print("# Flags per service: {}".format([cnt for id, cnt in flags_per_service]))
        highscores = (
            SubmittedFlag.query.with_entities(SubmittedFlag.submitted_by, func.count())
            .group_by(SubmittedFlag.submitted_by)
            .all()
        )
    ...
    def _init_services(self, service_count: int, **kwargs):
        for i in range(service_count):
            db_session().add(
                Service(name=f"Service {i + 1}", checker_script="", **kwargs)
            )

    def _submit_checker_results(
        self,
        endround: int,
        chance_offline=0.02,
        chance_flagmissing=0.02,
        chance_mumble=0.05,
        seed=1337,
    ):
    ...
    def _submit_random_flags(self, endround: int, chance=0.33, seed=1337):
        ...
        for s in services:
            ...
            for t in teams:
                team_exploit_ready[(t.id, s.id)] = int(
                    rnd.randint(1, round(endround / chance))
                    * service_difficulty
                    * team_strength[t.id]
                )
                team_patch_ready[(t.id, s.id)] = int(
                    rnd.randint(1, round(endround))
                    * service_difficulty
                    * team_strength[t.id]
                )
        for s in services:
            ...
            for attacker in teams:
                if team_exploit_ready[(attacker.id, sid)] > endround:
                    continue
                for victim in teams:
                    for r in range(
                        team_exploit_ready[(attacker.id, sid)],
                        min(endround, team_patch_ready[(victim.id, sid)]),
                    ):
                        # hack!
                        p = 0
                        if s.num_payloads > 1:
                            p = rnd.randint(0, s.num_payloads - 1)
                        flag = SubmittedFlag(
                            submitted_by=attacker.id,
                            service_id=sid,
                            team_id=victim.id,
                            tick_issued=r,
                            payload=p,
    ...
    def test_something_like_faust2019(self):
        ...
        print(
            f"average: {sum(times) / len(times):.3f} sec    min: {min(times):.3f} sec    max: {max(times):.3f} sec"
        )
        print(
            f"avg first 15: {sum(times[:15]) / len(times[:15]):.3f} sec         avg last 15: {sum(times[-15:]) / len(times[-15:]):.3f} sec"
        )
