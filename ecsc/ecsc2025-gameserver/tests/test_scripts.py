...
class ScriptsTests(DatabaseTestCase):
    ...
    def test_reset_ctf_to_round(self) -> None:
        ...
        result = ScriptRunner.run_script(
            "scripts/reset_ctf_to_round.py", ["0", "--force"]
        )
        ScriptRunner.assert_no_exception(result)
        self.assertIn(b"Done.", result.stdout)
    ...
    def test_recreate_scoreboard(self) -> None:
        ...
        with TemporaryDirectory() as directory:
            config.current_config.PUBLIC_SCOREBOARD_PATH = Path(directory)
            result = ScriptRunner.run_script("scripts/recreate_scoreboard.py", [])
            ScriptRunner.assert_no_exception(result)
            self.assertIn(b"Done.", result.stdout)
            self.assertTrue(
                (
                    config.config.PUBLIC_SCOREBOARD_PATH
                    / "api"
    ...
    def test_patch(self) -> None:
        ...
        with TemporaryDirectory() as directory:
            ...
            result = ScriptRunner.run_script(
                "scripts/patch_prepare.py", [str(f1), str(f2)]
            )
            ScriptRunner.assert_no_exception(result)
            ...
            result = ScriptRunner.run_script(
                "scripts/patch_publish.py", [str(f1), str(f2)]
            )
            ScriptRunner.assert_no_exception(result)
            private_files = [p.name for p in (base / "private").glob("*")]
            public_files = [p.name for p in (base / "public").glob("*")]
    ...
    def test_timer(self) -> None:
        result = ScriptRunner.run_script_for_time(
            "controlserver/master_timer.py", [], duration=2
        )
        ScriptRunner.assert_no_exception(result)
    ...
    def test_scoreboard_daemon(self) -> None:
        ...
        with TemporaryDirectory() as directory:
            config.current_config.PUBLIC_SCOREBOARD_PATH = Path(directory)
            result = ScriptRunner.run_script_for_time(
                "controlserver/scoring/scoreboard_process.py", [], duration=2
            )
            ScriptRunner.assert_no_exception(result)
