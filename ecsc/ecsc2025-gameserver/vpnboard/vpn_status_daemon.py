...
class VpnStatusThread(threading.Thread):
    ...
    def run(self) -> None:
        for _ in range(
            18
        ):  # wait at most 3 minutes if redis connections fail, then abort
            try:
...
class VpnStatusDaemon:
    ...
    def update_status(self, check_vulnboxes: bool) -> list[VpnStatus]:
        with db_session_2() as session:
            states = [
                VpnStatus(team) for team in session.query(Team).order_by(Team.id).all()
            ]
            if USE_WIREGUARD:
                self.update_team_wireguard_status(states)
                session.commit()
            self.ping_teams_threadpool(
                [s for s in states if s.connected], check_vulnboxes
            )
            session.expunge_all()
            return states
            ...
            for team_status in states:
                ...
                try:
                    ...
                    self._logger.warning(
                        f"Team {team.id} has no readable interface ({e})"
                    )
                ...
                vulnbox_peer = team_status.wg.get_peer_for(
                    config.NETWORK.team_id_to_vulnbox_ip(team.id)
                )
                if vulnbox_peer is not None:
                    if not team.wg_vulnbox_connected and vulnbox_peer.connected:
                        ...
                        if vulnbox_peer.last_handshake is not None:
                            team.vpn_last_disconnect = (
                                vulnbox_peer.last_handshake
                ...
                interesting_peers = [
                    vulnbox_peer,
                    team_status.wg.get_peer_for(
                        config.NETWORK.team_id_to_gateway_ip(team.id)
                    ),
                    team_status.wg.get_peer_for(
                        config.NETWORK.team_id_to_testbox_ip(team.id)
                    ),
                ]
                boxes_connected = any(
                    p is not None and p.connected for p in interesting_peers
                )
    ...
    def ping_teams_threadpool(
        self, states: list[VpnStatus], check_vulnboxes: bool = False
    ) -> None:
        """
        ...
        try:
            ...
            data = pool.imap_unordered(
                lambda s: (
                    ...
                    (
                        test_ping
                        if s.team.vpn2_connected or not self.use_nping
                        else test_nping
                    ...
                    ping_func(config.NETWORK.team_id_to_vulnbox_ip(s.team.id))
                    if check_vulnboxes
                    else None,
                ),
                states,
        ...
        for state, ping_router, ping_testbox, web_testbox, ping_vulnbox in data:
            state.router_ping_ms = (
                ping_router if isinstance(ping_router, float) else None
            )
            state.testbox_ping_ms = (
                ping_testbox if isinstance(ping_testbox, float) else None
            )
            state.vulnbox_ping_ms = (
                ping_vulnbox if isinstance(ping_vulnbox, float) else None
            )
    ...
    def run_once(self) -> None:
        ...
        for handler in self._handlers:
            handler.update(
                states, self._vpn_status.banned_teams, check_vulnboxes, start
            )

        duration = time.time() - start
        self._logger.info(
            f"Created VPN board, took {duration:.3f} seconds "
    ...
    def run_loop(self) -> None:
        ...
        while True:
            ...
            sleeptime = min(
                60, max(5, round((60 if duration >= 25 else 30) - duration))
            )
