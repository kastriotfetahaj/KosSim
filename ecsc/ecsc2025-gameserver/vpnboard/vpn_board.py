...
class TeamResult:
    def __init__(self) -> None:
        ...
        self.vulnbox_ping_ms: Optional[float] = (
            None  # only filled if explicitly selected
        )
...
class VpnBoard(VpnStatusHandler):
    ...
    def copy_static_files(self) -> None:
        static_dir: Path = (
            Path(os.path.abspath(__file__)).parent.parent / "controlserver" / "static"
        )
        shutil.copyfile(
            static_dir / "css" / "vpnboard.css", config.VPNBOARD_PATH / "index.css"
        )
        shutil.copyfile(
            static_dir / "img" / "favicon.png", config.VPNBOARD_PATH / "favicon.png"
        )

    def render_template(
        self, template: str, filename: str, minimize=False, **kwargs
    ) -> None:
        """
    ...
    def build_vpn_json(self, teams: Iterable[Team]) -> None:
            ...
            "teams": [
                {
                    ...
                    or team.vpn2_connected
                    or team.wg_boxes_connected,
        ...
        data = {
            "teams": [
                ...
                or team.vpn2_connected
                or team.wg_boxes_connected
                or team.vpn_last_connect is not None
            ]
        }
    ...
    def build_vpn_board(
        self,
        states: list[VpnStatus],
        banned_teams: set[int] | None = None,
        check_vulnboxes: bool = False,
        ...
        self.render_template(
            ...
            "vpn.html",
            minimize=True,
            wireguard=any(s.wg is not None for s in states),
            start=datetime.datetime.fromtimestamp(start, datetime.timezone.utc),
            states=states,
            check_vulnboxes=check_vulnboxes,
    ...
    def update(
        self,
        states: list[VpnStatus],
        banned_teams: set[int],
        check_vulnboxes: bool,
        start: float,
    ) -> None:
