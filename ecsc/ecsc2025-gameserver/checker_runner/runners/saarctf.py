...
class SaarctfServiceRunner(CheckerRunner):
    ...
    def get_service_config(self, service_id: int) -> ServiceConfig:
        if self.package in self._service_config_cache:
            ...
            with db_session_2() as session:
                ...
                fn, cl = (
                    service.checker_script.split(":", 1)
                    if service.checker_script
                    else ("", "")
                )
                cfg = ServiceConfig(
                    name=service.name,
    ...
    def _execute_checker_unchecked(
        self, service_id: int, team_id: int, tick: int
    ) -> CheckerRunOutput:
        """
        ...
        """
        team = gamelib.Team(
            team_id, "#" + str(team_id), config.NETWORK.team_id_to_vulnbox_ip(team_id)
        )
    ...
    def execute_checker(self, team_id: int, tick: int) -> CheckerRunOutput:
        try:
            result = handle_checker_exceptions(
                lambda: self._execute_checker_unchecked(self.service_id, team_id, tick)
            )
    ...
    def execute_checker_subprocess(
        ...
        try:
            ...
            return CheckerRunOutput(
                status, message=(message.strip() or None), output=output
            )
        except subprocess.TimeoutExpired as e:
            return CheckerRunOutput(
                "TIMEOUT",
                message="Timeout, service too slow",
