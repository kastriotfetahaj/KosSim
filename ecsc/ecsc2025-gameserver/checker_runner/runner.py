...
from checker_runner.checker_execution import (
    process_needs_restart,
    set_process_needs_restart,
    CheckerRunOutput,
)
from checker_runner.runners.factory import CheckerRunnerFactory
from controlserver.models import CheckerResult, init_database, db_session_2
...
def set_limits() -> None:
    ...
    if "SAARCTF_NO_RLIMIT" not in os.environ:
        resource.setrlimit(
            resource.RLIMIT_AS, (2048 * 1000000, 3072 * 1000000)
        )  # 2GB soft / 3GB hard
    pass
...
def save_checker_result(
    ...
    team_id: int,
    celery_id: str,
    result: CheckerRunOutput,
    runtime: float,
) -> None:
    dbresult = CheckerResult(
        tick=tick, service_id=service_id, team_id=team_id, celery_id=celery_id
    )
...
def run_checkerscript(
    ...
    try:
        save_checker_result(
            tick, service_id, team_id, self.request.id, result, time.time() - start_time
        )
...
def run_checkerscript_external(
    self,
    runner_spec: str,
    package: str,
    script: str,
    ...
    start_time = time.time()
    runner = CheckerRunnerFactory.build(runner_spec, service_id, package, script, cfg)
    result = runner.execute_checker_subprocess(
        team_id, tick, self.request.timelimit[0] - 5
    )
    save_checker_result(
        tick, service_id, team_id, self.request.id, result, time.time() - start_time
    )
...
def run_command(cmd: str) -> str:
    ...
    try:
        output = subprocess.check_output(
            cmd, stderr=subprocess.STDOUT, cwd=cwd, shell=True, timeout=90
        )
...
class CeleryWorker:
    ...
    def init(self, threadsafe=False) -> None:
        print(
            f"CONFIGURING CELERY: broker={config.celery_url()}, backend={config.celery_redis_url()}"
        )
        self.app = Celery(
        ...
        self.run_checkerscript_external = self.app.task(bind=True)(
            run_checkerscript_external
        )
        self.preload_packages = self.app.task(
            queue="broadcast", options=dict(queue="broadcast")
        )(preload_packages)
        self.run_command = self.app.task(
            queue="broadcast", options=dict(queue="broadcast"), soft_time_limit=100
        )(run_command)
