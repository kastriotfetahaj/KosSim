...
class UserAgentCollector:
    ...
    def collect_api(self) -> None:
        response = requests.get(
            "https://github.com/microlinkhq/top-user-agents/raw/refs/heads/master/src/index.json"
        )
    ...
    def collect_requests(self) -> None:
        ...
        releases = requests.get("https://pypi.org/pypi/requests/json").json()[
            "releases"
        ]
        version_strings = [
            k
            for k, v in releases.items()
            if not any(artifact["yanked"] for artifact in v)
        ]
