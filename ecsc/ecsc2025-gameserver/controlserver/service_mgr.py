...
class ServiceRepoManager:
    ...
    def import_services(self) -> None:
        ...
        with db_session_2() as session:
            by_name = {
                service.name: service for service in session.query(Service).all()
            }
    ...
    def _update_db_from_cfg(
        self, dir: Path, cfg: ServiceConfig, service: Service
    ) -> None:
        service.name = cfg.name
        ...
        with db_session_2() as session:
            ...
            if not dir.exists():
                ...
                if (
                    cfg.name in by_name
                    and cfg_before != cfg
                    and self._cfg_equals_db(dir, cfg_before, by_name[cfg.name])
                ):
                    self._update_db_from_cfg(dir, cfg, by_name[cfg.name])
                    self._report(
                        f"Updating service {by_name[cfg.name].id}: {by_name[cfg.name].name} because config changed"
                    )
    ...
    def update_all_services(self) -> None:
        ...
        if not self.is_owner():
            raise PermissionError(
                f"You are not the owner of {config.SERVICES_PATH}. Please do not mess up permissions."
            )

        for remote_service in config.SERVICE_REMOTES:
            self.update_service(remote_service)

    def upload_checker_scripts(
        self, checker_script_dir: str
        ...
        fnames = [
            f
            for f in [base / "dependencies.sh", base / "checker-requirements.txt"]
            if f.exists()
        ]
        if len(fnames) > 0:
