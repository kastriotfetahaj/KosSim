...
def init_database(app: Flask | None = None) -> None:
    engine = create_engine(
        config.postgres_sqlalchemy(), pool_size=50, max_overflow=10, echo=False
    )
...
class Team(Base, ModelMixin):
    ...
    )  # reference to TeamLogo.hash
    points = relationship("TeamPoints", back_populates="team")
    vpn_connected = mapped_column(
        Boolean, nullable=False, server_default=text("FALSE")
    )  # team-hosted VPN
    vpn2_connected = mapped_column(
        Boolean, nullable=False, server_default=text("FALSE")
    )  # cloud-hosted vulnbox VPN
    vpn_connection_count = mapped_column(
        Integer, nullable=False, server_default=text("0")
    )  # "cloud"-style VPN connections / wireguard active peers
    # OpenVPN selfhosted connect/disconnect timestamp, or wireguard connect timestamp for vulnbox ip
    vpn_last_connect = mapped_column(
        TIMESTAMP(timezone=True), nullable=True, server_default=text("NULL")
    )
    vpn_last_disconnect = mapped_column(
        TIMESTAMP(timezone=True), nullable=True, server_default=text("NULL")
    )
    wg_vulnbox_connected = mapped_column(
        Boolean, nullable=False, server_default=text("FALSE")
    )  # team has a wireguard connection for vulnbox ip
    wg_boxes_connected = mapped_column(
        Boolean, nullable=False, server_default=text("FALSE")
    )  # team has a wg conn for router, vulnbox, or testbox ip
...
class Service(Base, ModelMixin):
    ...
    checker_script_dir = mapped_column(
        String, nullable=True, server_default=text("NULL")
    )
    checker_script = mapped_column(
        String, nullable=False
    )  # filename:classname (relative to the classpath)
    package = mapped_column(
        String(32), nullable=True
    )  # package the checker files have been moved to
    setup_package = mapped_column(
        String(32), nullable=True, server_default=text("NULL")
    )  # package containing an init script

    # checker details
    checker_subprocess = mapped_column(
        Boolean, nullable=False, server_default=text("FALSE")
    )  # saarctf framework only
    checker_route = mapped_column(
        String(64), nullable=True, server_default=text("NULL")
    )
    runner_config = mapped_column(
        JSON, nullable=True, server_default=text("NULL")
    )  # additional properties for your custom runner

    # service details
    num_payloads = mapped_column(
        Integer, nullable=False, server_default=text("0")
    )  # number of possible payloads. 0 = unlimited
    flag_ids = mapped_column(
        String(128), nullable=True, server_default=text("NULL")
    )  # flag id types, comma-separated
...
class TeamPoints(Base, ModelMixin):
    ...
    team_id = mapped_column(
        SmallInteger, ForeignKey("teams.id", ondelete="CASCADE"), nullable=False
    )
    service_id = mapped_column(
        SmallInteger, ForeignKey("services.id", ondelete="CASCADE"), nullable=False
    )
    team_points_unique_1 = UniqueConstraint(
        "tick", "team_id", "service_id", name="team_points_unique_1"
    )
    __table_args__ = (team_points_unique_1,)

    # modify these columns to fit your scoring scheme (and check the methods below)
    flag_captured_count = mapped_column(
        Integer, nullable=False
    )  # captured BY this team
    ...
    def efficient_insert(
        cls,
        tick: int,
        items: typing.Collection["TeamPoints | TeamPointsLite"],
        ...
        sql = (
            "INSERT INTO team_points (team_id, service_id, tick, flag_captured_count, flag_stolen_count, off_points, def_points, sla_points, sla_delta) "
...
class TeamRanking(Base, ModelMixin):
    ...
    __table_args__ = (
        UniqueConstraint("tick", "team_id", name="team_rankings_unique_1"),
    )
...
class TeamTrafficStats(Base, ModelMixin):
    ...
    __table_args__ = (
        UniqueConstraint("time", "team_id", name="team_traffic_stats_unique_1"),
    )
...
class SubmittedFlag(Base, ModelMixin):
    ...
    id = mapped_column(Integer, primary_key=True)
    submitted_by = mapped_column(
        SmallInteger, nullable=False
    )  # references teams (attacking team)
    team_id = mapped_column(
        SmallInteger, nullable=False
    )  # references teams (exploited team)
    ...
    payload = mapped_column(
        Integer, nullable=False, server_default=text("0")
    )  # more or less random payload
    tick_submitted = mapped_column(
        SmallInteger, nullable=False, index=True
    )  # submitted in this tick
    is_firstblood = mapped_column(
    ...
    victim_team = relationship(
        "Team", foreign_keys=[team_id], primaryjoin="Team.id == SubmittedFlag.team_id"
    )
    ...
    def efficient_insert(cls, items) -> None:
        ...
        cursor = db_session().connection().connection.cursor()
        sql = (
            f"INSERT INTO {cls.__tablename__} (team_id, service_id, tick_issued, payload, submitted_by, tick_submitted) "
...
class CheckerResult(Base, ModelMixin):
    ...
    checker_results_unique_1 = UniqueConstraint(
        "tick", "team_id", "service_id", name="checker_results_unique_1"
    )
...
class CheckerResultLite:
    ...
    def efficient_insert(cls, items) -> None:
        ...
        cursor = db_session().connection().connection.cursor()
        sql = (
            f"INSERT INTO {CheckerResult.__tablename__} (team_id, service_id, tick, status, run_over_time, message, celery_id) "
...
class Tick(Base, ModelMixin):
    ...
    def _set(cls, session: Session, tick: int, field: str, ts: datetime) -> None:
        sql = (
            f'INSERT INTO {Tick.__tablename__} (tick, "{field}") VALUES (:tick, :ts) '
