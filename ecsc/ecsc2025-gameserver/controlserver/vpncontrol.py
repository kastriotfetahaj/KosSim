...
from controlserver.models import LogMessage, Team
from controlserver.utils.ctfroute import (
    ban_team,
    close_network,
    open_network,
    open_network_within_teams,
    open_network_within_teams_no_vulnbox,
    remove_ban,
)
    ...
    def set_state(self, state: VpnStatus) -> None:
        ...
        if old_state != state:
            if state == VpnStatus.ON:
                ...
                log(
                    "vpn",
                    "Network open within teams only, excluding vulnboxes",
            ...
            time.sleep(
                0.5
            )  # delay further CTF events (e.g. checker script dispatcher) until the firewall is really open
