...
def checker_results_index(page=1) -> ResponseReturnValue:
    query = CheckerResult.query.options(joinedload(CheckerResult.team)).options(
        joinedload(CheckerResult.service)
    )
    ...
    filter_status = (
        request.args["filter_status"].split("|")
...
def checker_results_view(id=None) -> ResponseReturnValue:
    checker_result: CheckerResult | None = (
        CheckerResult.query.options(joinedload(CheckerResult.team))
        .options(joinedload(CheckerResult.service))
        .filter(CheckerResult.id == id)
        .first()
    )
