...
def log_messages_index(page: int = 1) -> ResponseReturnValue:
    ...
    if filter_level is not None:
        query = query.filter(
            LogMessage.level.in_([getattr(LogMessage, level) for level in filter_level])
        )
    ...
    components = [
        (c, c)
        for (c,) in session.query(LogMessage.component.distinct())
        .order_by(LogMessage.component)
        .all()
    ]
...
def log_messages_view(id: int | None = None) -> ResponseReturnValue:
    ...
    return render_template(
        "log_messages_view.html", log_message=log_message, LogMessage=LogMessage
    )
