...
def teams_index(page: int = 1) -> ResponseReturnValue:
    ...
    if "." not in order:
        ...
        if order.startswith("sum."):
            ...
            cols = [
                column
                for name, column in TeamTrafficStats.__dict__.items()
    ...
    filter_online = (
        request.args["filter_level"].split("|")
        if "filter_level" in request.args
        else None
    )
    if filter_online is not None:
        conditions: list[Any] = []
        if "online" in filter_online:
            conditions.append(
                (Team.vpn_connected == True)
                | (Team.vpn2_connected == True)
                | (Team.wg_vulnbox_connected == True)
            )
        if "ever online" in filter_online:
            conditions.append(Team.vpn_last_connect != None)
        if "offline" in filter_online:
            conditions.append(
    ...
    current_stats_timestamp = (
        db_session().query(func.max(TeamTrafficStats.time)).scalar()
    )
    ...
    query = query.outerjoin(
        TeamTrafficStats,
        and_(
            Team.id == TeamTrafficStats.team_id,
            TeamTrafficStats.time == current_stats_timestamp,
        ),
    )
...
def teams_view(id: int | None = None) -> ResponseReturnValue:
    ...
    traffic_stats: List[TeamTrafficStats] = (
        TeamTrafficStats.query.filter(TeamTrafficStats.team_id == id)
        .order_by(TeamTrafficStats.time.desc())
        .limit(60)
        .all()[::1]
    )
    # "#97BBCD","#DCDCDC"
    graph1: list[dict[str, Any]] = [
        {"label": "team's download", "data": []},
        {"label": "team's upload", "data": []},
    ]
    graph2: list[dict[str, Any]] = [
        {"label": "team to team", "data": []},
        {"label": "total", "data": []},
    ]
    graph3: list[dict[str, Any]] = [
        {"label": "team to team", "data": []},
        {"label": "total", "data": []},
    ]
    labels: list[float] = []
