...
def flags_index() -> ResponseReturnValue:
    ...
    if flag:
        try:
            ...
            real_mac_bin = hmac.HMAC(
                config.SECRET_FLAG_KEY, data[:8], hashlib.sha256
            ).digest()[:MAC_LENGTH]
            ...
            if valid_except_mac and not valid:
                repaired_flag = (
                    config.FLAG_PREFIX
                    + "{"
                    + base64.b64encode(data[:8] + real_mac_bin).decode()
                    + "}"
                )
        ...
        return render_template(
            "flags.html", flag=None, flag_regex=flag_regex, demo_flags=demo_flags
        )
