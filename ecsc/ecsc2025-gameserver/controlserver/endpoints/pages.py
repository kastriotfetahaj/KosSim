...
from sqlalchemy import Integer, distinct, func, text
...
def frontpage() -> ResponseReturnValue:
    ...
    missing_scripts = (
        ...
        .filter(Service.checker_script_dir.isnot(None))
        .filter(Service.package.is_(None))
        .all()
    )
    scoreboard_paths = (config.PUBLIC_SCOREBOARD_PATH, config.INTERNAL_SCOREBOARD_PATH)
    missing_scoreboard = not all(
        (p / "api" / "scoreboard_current.json").exists() for p in scoreboard_paths
    )
    ...
    return render_template(
        ...
        scoreboard_path=config.INTERNAL_SCOREBOARD_PATH,
    )
...
def overview_logs(minId: Optional[int] = None) -> ResponseReturnValue:
    if minId is not None:
        logs = (
            LogMessage.query.filter(LogMessage.level >= LogMessage.INFO)
            .filter(LogMessage.id > minId)
...
def overview_components() -> ResponseReturnValue:
    clients = [
        client
        for client in get_redis_connection().client_list()
        if int(client["db"]) == config.REDIS["db"]
    ]
...
def overview_vpn() -> ResponseReturnValue:
    ...
    banned_teams = [
        {
            "id": team.id,
            "name": team.name,
            "network": config.NETWORK.team_id_to_network_range(team.id),
            "tick": tick,
        }
    ...
    open_teams = [
        ...
        for team in vpn.get_open_teams()
    ]
    teams_online = Team.query.filter(
        (Team.vpn_connected == True)
        | (Team.vpn2_connected == True)
        | (Team.wg_vulnbox_connected == True)
    ).count()
    teams_online_once = Team.query.filter(
        (Team.vpn_connected == False)
        & (Team.vpn2_connected == False)
        & (Team.wg_vulnbox_connected == False),
        Team.vpn_last_connect != None,
    ).count()
    teams_offline = Team.query.filter(
        (Team.vpn_connected == False)
        & (Team.vpn2_connected == False)
        & (Team.wg_vulnbox_connected == False),
        Team.vpn_last_connect == None,
    ...
    trafficstats_keys = []
    for line in (
        TeamTrafficStats.query_sum_lite()
        .filter(TeamTrafficStats.time > datetime.datetime.fromtimestamp(last))
        .filter(TeamTrafficStats.time <= text("NOW()"))
        .filter(TeamTrafficStats.time >= text("NOW() - interval '12 hours'"))
        .group_by(TeamTrafficStats.time)
        .order_by(TeamTrafficStats.time.desc())
        .limit(60)
        .all()
    ):
...
def packages() -> ResponseReturnValue:
    session = db_session()
    package_count = session.query(
        func.count(distinct(CheckerFilesystem.package))
    ).scalar()
    file_count, file_size = session.query(
        func.count(CheckerFile.id), func.sum(func.length(CheckerFile.content))
    ).first()  # type: ignore
    return render_template(
        "packages.html",
...
def packages_update() -> ResponseReturnValue:
    ...
    if "service" in body and body["service"]:
        services: List[Service] = Service.query.filter(
            Service.id == int(body["service"])
        ).all()
    elif "service_file" in body:
        services = (
            Service.query.filter(text(":filename LIKE checker_script_dir || '%'"))
            .params(filename=body["service_file"])
            .all()
        )
    else:
    ...
    for service in services:
        if service.checker_script_dir:
            if not os.path.exists(service.checker_script_dir):
                messages.append(
                    f'[{service.name}] Folder does not exist: "{service.checker_script_dir}"'
                )
                continue
            # upload package
            package, setup_package, is_new = service_mgr.upload_checker_scripts(
                service.checker_script_dir
            )
            # set if changed
            if package != service.package:
                if is_new:
                    messages.append(
                        "[{}] Created new package: {}".format(service.name, package)
                    )
                else:
                    messages.append(
                        "[{}] Switching to package: {}".format(service.name, package)
                    )
            ...
            if not os.path.exists(
                os.path.join(service.checker_script_dir, checker_script)
            ):
                messages.append(
            ...
            if setup_package != service.setup_package:
                ...
                messages.append(
                    f"[{service.name}] Init script from package: {setup_package}"
                )

    # notify workers, so they will preload these packages
    if changed_packages:
        celery_worker.preload_packages.apply_async(
            args=(changed_packages,), time_limit=120, soft_time_limit=100
        )
...
def packages_push() -> ResponseReturnValue:
    ...
    celery_worker.preload_packages.apply_async(
        args=(packages,), time_limit=90, soft_time_limit=60
    )
...
def packages_run() -> ResponseReturnValue:
    ...
    if command:
        task = celery_worker.run_command.apply_async(
            args=(command,), time_limit=120, soft_timeout=100
        )
...
def packages_test() -> ResponseReturnValue:
    ...
    if "service_id" in body:
        ...
        try:
            ...
            return Response(
                f'File "{body["service_file"]}" does not belong to a known service',
                status=400,
            )
    ...
    if not tick:
        min_round = (
            db_session()
            .query(func.min(CheckerResult.tick))
            .filter(CheckerResult.team_id == team.id)
            .filter(CheckerResult.service_id == service.id)
            .scalar()
        )
...
def checker_status(tick: int | None = None) -> ResponseReturnValue:
    ...
    session = db_session()
    count = (
        session.query(func.count(CheckerResult.id))
        .filter(CheckerResult.tick == tick)
        .filter(CheckerResult.status != "PENDING")
        .filter(CheckerResult.status != "REVOKED")
        .scalar()
    )
    combinations = Dispatcher.default.get_tick_combinations(tick)
    if not combinations:
        return render_template(
            "404.html", message="Round {} has not yet been dispatched.".format(tick)
        ), 404
    services = Service.query.order_by(Service.name).all()
    ...
    results: List[CheckerResult] = (
        ...
        .filter(CheckerResult.tick == tick)
        .order_by(CheckerResult.finished)
        .all()
    )
    stats_dispatched: dict[int, int] = defaultdict(lambda: 0)  # service => count
    stats_results: dict[int, int] = defaultdict(
        lambda: 0
    )  # service => count of results
    stats_finished: dict[int, int] = defaultdict(
        lambda: 0
    )  # service => count of non-pending results
    stats_time: dict[int, float] = defaultdict(
        lambda: 0.0
    )  # service => sum of execution time
    stats_time_count: dict[int, int] = defaultdict(
        lambda: 0
    )  # service => number of execution times
    stats_toolate: dict[int, int] = defaultdict(
        lambda: 0
    )  # service => number of too lates
    stats_status: dict[int, dict[str, int]] = defaultdict(
    ...
    for service in services:
        stats_status[service.id]["PENDING"] += (
            stats_dispatched[service.id] - stats_results[service.id]
        )
    ...
    for r in results:
        if r.finished is not None:
            ...
            first_finished = (
                ts1 if not first_finished or ts1 < first_finished else first_finished
            )
            ts2 = int(ceil(r.finished.timestamp()))
            last_finished = (
                ts2 if not last_finished or ts2 > last_finished else last_finished
            )
    ...
    i = 0
    finished_per_tick.append(
        (datetime.datetime.fromtimestamp(t), defaultdict(lambda: 0))
    )
    for result in results:
            ...
            while result.finished.timestamp() >= t:
                ...
                finished_per_tick.append(
                    (
                        ...
                        copy.copy(finished_per_tick[i][1]),
                    )
                )
                i += 1
            finished_per_tick[i][1][
                result.status if not result.run_over_time else "TOOLATE"
            ] += 1
    while t < tick_end and t < time.time():
        t += bucketsize
        finished_per_tick.append(
            (datetime.datetime.fromtimestamp(t), copy.copy(finished_per_tick[i][1]))
        )
        i += 1
...
def checker_status_overview() -> ResponseReturnValue:
    ...
    results_ok = (
        session.query(
            CheckerResult.tick,
            func.count(CheckerResult.id),
            func.sum(CheckerResult.run_over_time.cast(Integer)),
        )
        .group_by(CheckerResult.tick)
        .filter(CheckerResult.tick >= first_tick)
        .filter(CheckerResult.tick <= last_tick)
        .filter(
            CheckerResult.status.in_(
                ["SUCCESS", "FLAGMISSING", "MUMBLE", "OFFLINE", "RECOVERING"]
            )
        )
        .all()
    )
    results_warn = (
        session.query(CheckerResult.tick, func.count(CheckerResult.id))
        .group_by(CheckerResult.tick)
        .filter(CheckerResult.tick >= first_tick)
        .filter(CheckerResult.tick <= last_tick)
        .filter(CheckerResult.status == "TIMEOUT")
        .all()
    )
    results_error = (
        session.query(CheckerResult.tick, func.count(CheckerResult.id))
        ...
        .filter(CheckerResult.tick >= first_tick)
        .filter(CheckerResult.tick <= last_tick)
        .filter(CheckerResult.status == "CRASHED")
        .all()
    )
    results_revoked = (
        session.query(CheckerResult.tick, func.count(CheckerResult.id))
    ...
    results_last_finished = (
        session.query(CheckerResult.tick, func.max(CheckerResult.finished))
        .group_by(CheckerResult.tick)
        .filter(CheckerResult.tick >= first_tick)
        .filter(CheckerResult.tick <= last_tick)
        .all()
    ...
    for i in range(first_tick, Timer.current_tick + 1):
        ticks[i] = {
            ...
            "start": datetime.datetime.fromtimestamp(
                int(redis.get("round:{}:start".format(i)) or 0), datetime.timezone.utc
            ),  # type: ignore
            "end": datetime.datetime.fromtimestamp(
                int(redis.get("round:{}:end".format(i)) or 0), datetime.timezone.utc
            ),  # type: ignore
    ...
    return render_template(
        "checker_status_overview.html", ticks=ticks, first_tick=first_tick
    )
