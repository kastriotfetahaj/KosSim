...
def patches_index() -> ResponseReturnValue:
    ...
    files_with_url: list[tuple[str, str | None]] = [
        (
            f,
            config.PATCHES_URL + "/" + f
            if config.PATCHES_URL and f in public_files
            else None,
        )
        for f in files
    ]
    ...
    if request.method == "POST":
        ...
        command = ANSIBLE_COMMAND + [
            fname,
            "-i",
            "hosts_nop.yaml" if is_test else "hosts.yaml",
            "-f",
            "32",
        ]
        # build target files
        ...
        try:
            output_raw = subprocess.check_output(
                command, stderr=subprocess.STDOUT, timeout=90, cwd=config.PATCHES_PATH
            )
            output = (
                "> "
                + " ".join(command)
        ...
        return render_template(
            ...
            patches_public_path=config.PATCHES_PUBLIC_PATH,
