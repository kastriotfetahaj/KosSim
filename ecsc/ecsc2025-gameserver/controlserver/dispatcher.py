...
class Dispatcher:
        ...
        with db_session_2() as session:
            with get_redis_connection() as redis:
                ...
                services = (
                    ...
                    .order_by(Service.id)
                    .filter(Service.checker_enabled == True)
                    .all()
                )
                combinations = list(itertools.product(teams, services))
                if len(combinations) > 0:
                    random.shuffle(combinations)
                    taskgroup_sig: Signature = group(
                        [
                            self.__prepare_checker_script(team, service, tick)
                            for team, service in combinations
                        ]
                    )
    ...
    def dispatch_test_script(
        self, team: Team, service: Service, tick: int, package: str | None = None
    ) -> tuple[AsyncResult, CheckerResult]:
        session = db_session()  # operation called from cp only
        # cleanup database
        session.query(CheckerResult).filter(
            CheckerResult.team_id == team.id,
            CheckerResult.service_id == service.id,
            CheckerResult.tick == tick,
        ).delete()
        result = CheckerResult(
            team_id=team.id, service_id=service.id, tick=tick, status="PENDING"
        )
        session.add(result)
        # dispatch
        sig = self.__prepare_checker_script(
            team, service, tick, package, route=service.checker_route or "tests"
        )
    ...
    def dispatch_test_script_many(
        ...
        *,
        ...
        taskgroup_sig: Signature = group(
            [
                self.__prepare_checker_script(
                    team, service, tick, package=package, route=route
                )
                for tick in ticks
            ]
        )
    ...
    def get_tick_taskgroup(self, tick: int) -> GroupResult | None:
        if tick not in self.tick_taskgroups:
            ...
            if not group_id:
                return None
            self.tick_taskgroups[tick] = GroupResult.restore(
                group_id, app=celery_worker.app
            )
    ...
    def collect_checker_results(self, tick: int) -> None:
        ...
        with db_session_2() as session:
            ...
            if taskgroup and combinations:
                for (team_id, service_id), result in zip(
                    combinations, taskgroup.results
                ):
                    status = self.__collect_checker_result(
                        session, team_id, service_id, tick, result
                    )
                    stats[status] += 1
                session.commit()
                ...
                if stats[states.REVOKED] > 0:
                    log(
                        ...
                        "Not all checker scripts have been executed. {} / {} revoked, {} / {} still active".format(
                            stats[states.REVOKED],
                            len(combinations),
                            stats[states.STARTED],
                            len(combinations),
                        ),
                    ...
                    log(
                        "dispatcher",
                        "Not all checker scripts finished in time: {} / {} still active".format(
                            stats[states.STARTED], len(combinations)
                        ),
                        level=LogMessage.WARNING,
                    )
                else:
                    last_finished = (
                        session.query(func.max(CheckerResult.finished))
                        .filter(CheckerResult.tick == tick)
                        .scalar()
                    )
                    if (
                        last_finished
                        and collect_time - last_finished.timestamp() <= 3.5
                    ):
                        log(
                            "dispatcher",
                            "Worker close to overload: Last checker script finished {:.1f} sec before deadline".format(
                                collect_time - last_finished.timestamp()
                            ),
                            level=LogMessage.WARNING,
                        )
            ...
            # print(stats)
            crashed = (
                session.query(CheckerResult.service_id, func.count())
                .filter(CheckerResult.tick == tick)
                .filter(CheckerResult.status == "CRASHED")
                .group_by(CheckerResult.service_id)
                .all()
            )
            for service_id, count in crashed:
                ...
                log(
                    "dispatcher",
                    f"Checker scripts for {service.name if service else service_id} produced {count} errors in tick {tick}",
                    level=LogMessage.ERROR,
                )

    def __collect_checker_result(
        self,
        session: Session,
        team_id: int,
        service_id: int,
        ...
        if status == states.FAILURE:
            ...
            is_timeout = isinstance(r, Exception) and "TimeLimitExceeded" in repr(
                type(r)
            )
            db_result = CheckerResult(
                tick=tick, service_id=service_id, team_id=team_id, celery_id=result.id
            )
            if is_timeout:
                ...
                old_result = (
                    ...
                    .filter(
                        CheckerResult.tick == tick,
                        CheckerResult.service_id == service_id,
    ...
    def collect_test_results_many(
        self, team: Team, service: Service, ticks: list[int], taskgroup: GroupResult
    ) -> dict[str, int]:
        with db_session_2() as session:
            stats = {
                states.SUCCESS: 0,
            ...
            for tick, result in zip(ticks, taskgroup.results):
                status = self.__collect_checker_result(
                    session, team.id, service.id, tick, result
                )
                stats[status] += 1
            session.commit()
            return stats
