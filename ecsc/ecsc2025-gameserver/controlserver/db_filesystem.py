...
class DBFilesystem:
    ...
    def move_folder_to_package(self, folder: str) -> Tuple[str, bool]:
        ...
        package = hashlib.md5(
            json.dumps(file_information).encode("utf8"), usedforsecurity=False
        ).hexdigest()
        # package already exists?
        if (
            CheckerFilesystem.query.filter(CheckerFilesystem.package == package).count()
            > 0
        ):
    ...
    def load_package_to_folder(
        self, package: str, folder: Path, lfs_path: Path | None = None
    ) -> bool:
        """
        ...
        filesystem: List[CheckerFilesystem] = (
            CheckerFilesystem.query.filter(CheckerFilesystem.package == package)
            .order_by(CheckerFilesystem.path)
            .all()
        )
        for fs in filesystem:
            if fs.file_hash:
                ...
                if lfs_path and os.path.exists(lfs_path / fs.file_hash):
                    ...
                    file: CheckerFile = CheckerFile.query.filter(
                        CheckerFile.file_hash == fs.file_hash
                    ).one()
