...
class TickInformation:
    def __init__(
        ...
        team_points: dict[tuple[int, int], TeamPointsLite],
        checker_results: dict[tuple[int, int], CheckerResultLite],
    ) -> None:
        self.ticknumber: int = ticknumber
        self.ranking: list[TeamRanking] = ranking
        self.ranking_by_team_id: dict[int, TeamRanking] = {
            r.team_id: r for r in ranking
        }
    ...
    def get_attacker_victim_count(
        self, previous_info: Self
    ) -> tuple[dict[int, int], dict[int, int]]:
        # precompute attacker/victim count
        attacker_count: dict[int, int] = defaultdict(
            lambda: 0
        )  # service_id => team_count
...
class StatisticJsonGenerator(ABC):
    ...
    def update_file(
        ...
        servicenames: list[str] = [
            service.name if info.ticknumber >= 0 else "???" for service in services
        ]
        ...
        if info.ticknumber >= 0:
            ...
            if (
                ...
                if (
                    len(rows) != len(servicenames)
                    or min(len(row) for row in rows) <= info.ticknumber
                ):
                    rows = [self._empty_row(info.ticknumber + 1) for _ in services]
                if info.ticknumber > 0:
                    data["services"] = servicenames
                # fetch data from tick 0 up to now, and write to points table
                for (tick, service_id), result in self.get_all_tick_info(
                    services, info.ticknumber
                ).items():
...
class Scoreboard:
    ...
    def __init__(self, calculation: ScoringCalculation, *, output: Path | None = None, publish: bool = False, public: bool = True) -> None:
        ...
        self.output = output or (config.PUBLIC_SCOREBOARD_PATH if public else config.INTERNAL_SCOREBOARD_PATH)
    ...
    def create_scoreboard(
        self, ticknumber: int, has_started: bool = True, is_live: bool = False
    ) -> None:
        """
        ...
        with self._lock():
            if scoreboard_tick is None:
                old_data = self._read_json(
                    "api/scoreboard_current.json", {"scoreboard_tick": -1}
                )
            ...
            data = {
                ...
                "banned_teams": [
                    int(b.decode()) for b in self.conn.smembers("network:banned")
                ],
            }
    ...
    def __create_json_for_tick(
        ...
        for ranking in info.ranking:
            ...
            for service in self.services:
                ...
                service_points = (pts.off_points + pts.def_points) * pts.sla_points
                prev_service_points = (
                    prev_pts.off_points + prev_pts.def_points
                ) * prev_pts.sla_points
                if frozen:
                    services.append(
                        {
                            "p": prev_service_points,
                    ...
                    services.append(
                        {
                            ...
                            "d": pts.def_points,
                            "s": pts.sla_points,
                            "dp": service_points - prev_service_points,
                ...
                data["scoreboard"].append(
                    {
                        ...
                        "o": sum(last_off),
                        "d": sum(last_def),
                        "s": sum(last_sla) / max(len(last_sla), 1),
                        "do": 0,
                        "dd": 0,
                        "ds": 0,
                    }
                ...
                data["scoreboard"].append(
                    {
                        ...
                        "services": services,
                        "dp": ranking.points
                        - previous_info.ranking_by_team_id[ranking.team_id].points,
                        "o": off_points,
                        "d": def_points,
                        "s": sla_points / max(len(self.services), 1),
                        "do": off_points - prev_off_points,
                        "dd": def_points - prev_def_points,
                        "ds": (sla_points - prev_sla_points)
                        / max(len(self.services), 1),
                    }
                )
        ...
        if frozen:
            ...
            frozen_first_blood_info = self.__get_first_blood_info(
                previous_info.ticknumber
            )
            data["services"] = [
                {
                    "name": service.name,
    ...
    def __create_json_for_teams(
        self,
        info: TickInformation,
        previous_info: TickInformation,
        scoreboard_freeze: bool,
    ) -> None:
        """
    ...
    def __create_service_stat_json(
        self,
        info: TickInformation,
        previous_info: TickInformation,
        scoreboard_freeze: bool,
        ...
        with self._lock():
            ...
            gen.update_file(
                "api/scoreboard_service_stats.json", self.services, info, previous_info
            )
    ...
    def create_ctftime_json(self, ticknumber: int) -> str:
        ...
        with db_session_2() as session:
            ...
            team_names = {
                team_id: name
                for team_id, name in session.query(Team.id, Team.name).all()
            }
            data = {
                "standings": [
                    {
                        "pos": ranking.rank,
                        "team": team_names[ranking.team_id],
                        "score": round(ranking.points, 4),
                    }
                    for ranking in rankings
                    if ranking.points > 0
                    and ranking.team_id != config.SCORING.nop_team_id
                ]
            }
    ...
    def __fetch_data(self, session: Session, ticknumber: int) -> TickInformation:
        return TickInformation(
            ...
            self.calculation.get_results_for_tick_lite(
                session, ticknumber, [team.id for team in self.teams]
            ),
    ...
    def __get_first_blood_info(self, ticknumber: int) -> dict[int, tuple[list[str], set[int]]]:
        ...
        with db_session_2() as session:
            ...
            flags: list[SubmittedFlag] = (
                session.query(SubmittedFlag)
                .filter(
                    SubmittedFlag.is_firstblood == True,
                    SubmittedFlag.tick_submitted <= ticknumber,
                )
...
def default_scoreboards(calculation: ScoringCalculation, *, publish: bool = False) -> list[Scoreboard]:
    scoreboards = []
    if config.PUBLIC_SCOREBOARD_PATH:
        scoreboards.append(Scoreboard(calculation, public=False, publish=publish and not config.INTERNAL_SCOREBOARD_PATH))
    if config.INTERNAL_SCOREBOARD_PATH:
        scoreboards.append(Scoreboard(calculation, public=True, publish=publish))
    return scoreboards
