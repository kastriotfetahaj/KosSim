...
def log(
    component: str, title: str, text: str = "", level: int = LogMessage.INFO
) -> None:
...
def log_exception(component: str, e: Exception, errormessage: str = "{}: {}") -> None:
    ...
    :return:
    """
    msg = errormessage.format(
        e.__class__.__name__, e.args[0] if len(e.args) >= 1 else ""
    )
