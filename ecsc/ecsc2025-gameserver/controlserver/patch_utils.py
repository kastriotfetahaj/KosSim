...
class PatchUtils:
    def __init__(
        self, patchfile: Path | str, additional_files: List[Path] | None = None
    ) -> None:
        ...
        for i in range(len(self.additional_files)):
            if not os.path.exists(self.additional_files[i]):
                self.additional_files[i] = (
                    config.PATCHES_PATH / self.additional_files[i]
                )
    ...
    def ansible_base_template() -> str:
        return (
            ANSIBLE_TEMPLATE_1
            + ANSIBLE_TEMPLATE_2.replace("<FNAME>", "your script")
            + ANSIBLE_TEMPLATE_3
        )
    ...
    def create_ansible_template(self) -> Path:
        ...
        if not fname.exists():
            ...
            tmpl += (
                ANSIBLE_TEMPLATE_2.replace("<PATH1>", os.path.abspath(self.patchfile))
            ...
            for f in self.additional_files:
                tmpl += (
                    ANSIBLE_TEMPLATE_2.replace("<PATH1>", os.path.abspath(f))
    ...
    def get_ansible_hosts_templates() -> Tuple[str, str]:
        ...
        if config.DISPATCHER_CHECK_VPN_STATUS:
            qry = qry.filter(
                ...
                | (Team.wg_vulnbox_connected)
            )
        online_teams = qry.order_by(Team.id).all()
    ...
    def publish_patch_files(self) -> List[str]:
        ...
        for fname in [self.patchfile] + self.additional_files:
            ...
            urls.append(
                (config.PATCHES_URL or "/patches/") + "/" + os.path.basename(fname)
            )
        return urls
