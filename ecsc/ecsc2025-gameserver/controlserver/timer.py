...
def redis_set_and_publish(
    key: str, value: str | bytes | int | None, redis: StrictRedis | None = None
) -> None:
    if value is None:
        value = b"None"
...
class CTFTimerBase(ABC):
    def __init__(self) -> None:
        ...
        self._tick_time: int = config.TICK_DURATION
    ...
    def init(
        ...
        self.desired_state = CTFState[
            desired_state.decode("utf-8")
            if isinstance(desired_state, bytes)
            else desired_state
        ]
...
class CTFTimer(CTFTimerBase):
    ...
    def check_time(self) -> None:
        ...
        if self.state == CTFState.RUNNING and self._tick_end and t >= self._tick_end:
            ...
            and self.start_at
            and self.start_at <= t <= self.start_at + 4
        ):
            self._start_at = None
            self.start_ctf()
