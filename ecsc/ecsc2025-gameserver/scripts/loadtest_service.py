...
class LoadTestResult:
    ...
    def __post_init__(self) -> None:
        ...
        self.times = TimeStats.from_times(
            [cast(float, result.time) for result in self.results]
        )
        ...
        walltime_start = min(
            result.finished - timedelta(seconds=cast(float, result.time))
            for result in self.results
            if result.finished is not None and result.time is not None
        )
        walltime_end = min(
            result.finished for result in self.results if result.finished is not None
        )
        self.walltime = (walltime_end - walltime_start).total_seconds()

    def to_line(self) -> str:
        return (
            f"n={n}  success={self.success}/{len(self.results)}  runtime={self.times}"
        )
...
class LoadTester:
    ...
    def _get_min_tick(self) -> int:
        with db_session_2() as session:
            min_tick: int | None = (
                session.query(func.min(CheckerResult.tick))
    ...
    def run(self, n: int) -> LoadTestResult:
        ...
        taskgroup = self.dispatcher.dispatch_test_script_many(
            self.team, self.service, ticks, route="loadtest"
        )
        taskgroup.join()
        self.dispatcher.collect_test_results_many(
            self.team, self.service, ticks, taskgroup
        )
        with db_session_2() as session:
            results = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--service", type=int, required=True, help="Service ID to load-test"
    )
    parser.add_argument(
        "-n",
        type=str,
        default="1,2,4,8,16,32,48,64",
        help="Number of tasks (comma separated)",
    )
    ...
    logging.info(
        f"Testing service #{lt.service.id} {lt.service.name!r} against team #{lt.team.id} {lt.team.name!r}:"
    )
