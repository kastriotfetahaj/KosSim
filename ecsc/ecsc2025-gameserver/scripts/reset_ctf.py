...
def reset_database(include_storage=False):
    ...
    if include_storage:
        for m in ["CheckerFile", "CheckerFilesystem"]:
            count = getattr(controlserver.models, m).query.delete()
            print("- dropped {} entries from {}".format(count, m))
        controlserver.models.Service.query.update(
            {controlserver.models.Service.package: None}
        )
    controlserver.models.db_session().commit()


def reset_scoreboard():
    for path in (config.PUBLIC_SCOREBOARD_PATH, config.PUBLIC_SCOREBOARD_PATH):
        path /= "api"
        if path.exists():
            shutil.rmtree(path)
...
def reset_ctf(force: bool = False):
    if not force and not query_yes_no(
        "Do you really want to wipe the whole CTF?", "no"
    ):
        return
