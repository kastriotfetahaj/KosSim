...
class FlowerInterface:
    ...
    def get_worker_pool_size(self) -> dict[str, int]:
        workers = self.session.get(
            config.FLOWER_INTERNAL_URL + "api/workers", params={"refresh": "1"}
        ).json()
        return {
            name: len(worker["stats"]["pool"]["processes"])
    ...
    def get_worker_pool_size_for_queue(self, queue: str) -> int:
        ...
        for name, worker in workers.items():
            if (
                "stats" in worker
                and "active_queues" in worker
                and any(q["name"] == queue for q in worker["active_queues"])
            ):
    ...
    def get_worker_online(self) -> dict[str, bool]:
        workers = self.session.get(
            config.FLOWER_INTERNAL_URL + "dashboard?json=1"
        ).json()
...
def print_workers(
    workers: Dict[str, int], online: Dict[str, bool] | None = None
) -> None:
