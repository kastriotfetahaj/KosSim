...
def generate_team_password(team_id: int) -> str:
    if config.CONFIG_FILE:
        folder = str(
            Path(config.CONFIG_FILE).parent.name
        )  # for example "2020" or "test"
    else:
        folder = "none"
    secret = f"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA|{folder}|{team_id}"
        ...
        for team in teams:
            with open(
                os.path.join(
                    config.VPN_BASE_DIR,
                    "config-client",
                    f"client-team{team.id}-vulnbox.conf",
                ),
                "r",
            ) as f:
                vpnconfig = f.read()
            vm = first(
                ...
                if vm["kind"] == kind and vm["team"] == team.id
            )
            if not vm:
...
if __name__ == "__main__":
    ...
    simple_cloud_status(
        kinds, reset="--reset" in sys.argv, running="--create-only" not in sys.argv
    )
