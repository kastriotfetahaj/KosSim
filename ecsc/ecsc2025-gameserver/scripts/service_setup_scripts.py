...
def run_service_setup_scripts(filters: List[str]) -> int:
    ...
    if filters:
        services = (
            Service.query.filter(Service.name.in_(filters))
            .filter(Service.setup_package != None)
            .all()
        )
    else:
    ...
    for service in services:
        ...
        dbfs.load_package_to_folder(
            setup_package, path, Path("/tmp/setup_packages/lfs")
        )
        ...
        if not requirements_file.exists():
            print(
                f'[{service.name}] Requirements file not found: "{requirements_file}"!'
            )
            errors += 1
        else:
