...
def open_bpfmap(fname: str) -> int:
    ...
    while fd < 0 and BPF_RETRY_COUNT > 0:
        logging.info(
            f"[-] BPF map {fname.split('/')[-1]!r} not available, retry later ..."
        )
        BPF_RETRY_COUNT -= 1
        time.sleep(10)
        fd = libbpf.bpf_obj_get(fname.encode())
...
def save_difference(
    timestamp: int,
    new_results: Dict[int, List[int]],
    last_minute_results: Dict[int, List[int]],
) -> None:
    stuff_to_save = {}
...
def check_for_suspicious_numbers(
    new_results: Dict[int, List[int]], traffic_last_tick: Dict[int, List[int]]
) -> None:
    ...
    for team_id, traffic in traffic_last_tick.items():
        ...
        if team_upload > 5500000 * TICK_TIME:
            log(
                "traffic-control",
                f"Team #{team_id} uploaded too much last minute ({team_upload / 1000000:.1f} MB)",
                level=LogMessage.IMPORTANT,
            )
...
def main():
    ...
    try:
        ...
        if (
            loaded_data
            and loaded_data["last_minute_tick"]
            == floor(time.time() / TICK_TIME) * TICK_TIME
        ):
            ...
            logging.info(
                f"[OK] Initialized from saved state, {len(last_minute_results)} teams."
            )
            ...
            team_ids = [
                ...
                for r in db_session()
                .query(Team.id)
                .filter(Team.vpn_last_connect != None)
                .all()
            ]
        ...
        try:
            while True:
                wait_for_next_tick(1)
                t = time.time()
                t_display = datetime.datetime.utcfromtimestamp(t).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                ...
                if old_vpn_status or new_vpn_status:
                    save_difference(
                        round(t / TICK_TIME) * TICK_TIME,
                        new_results,
                        last_minute_results,
                    )
                    t = time.time() - t
                    logging.info(
                        f"[OK] Saved stats of {len(team_ids)} teams in {t:.3f} seconds."
                    )
                else:
                    logging.info("[-]  VPN is offline, no stats taken.")
