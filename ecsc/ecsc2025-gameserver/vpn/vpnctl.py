...
class SystemCtlUtils:
    ...
    def _execute(cls, cmd: list[str], check: bool = True) -> str:
        ...
        result = subprocess.run(
            ["systemctl"] + cmd, check=check, stdout=subprocess.PIPE
        )
    ...
    def is_active(cls, service_names: list[str]) -> dict[str, bool]:
        ...
        for offset in range(0, len(service_names), 100):
            ...
            text: str = cls._execute(
                ["is-active"] + service_names[offset : offset + 100], check=False
            )
            for i, line in enumerate(text.strip().split("\n")):
                if line == "active":
                    ...
                    print(
                        CliColors.WARNING
                        + f"[WARN] Service {service_names[offset + i]} is activating"
                        + CliColors.ENDC
                    )
                    ...
                    raise Exception(
                        f"Unexpected line in systemctl is-active output: {repr(line)}"
                    )
        if len(results) != len(service_names):
            raise Exception(
                "Unexpected length difference in systemctl is-active outputs: "
    ...
    def from_teams(cls, teams: list[Team]) -> list[VpnStatus]:
        ...
        return [
            VpnStatus(
                ...
                self_active=service_status[
                    SystemCtlUtils.service_vpn_selfhosted(team.id)
                ],
                cloud_active=service_status[
                    SystemCtlUtils.service_vpn_cloudhosted(team.id)
                ],
                cloud_players_active=service_status[
                    SystemCtlUtils.service_vpn_cloud_players(team.id)
                ],
            )
            for team in teams
        ]
    ...
    def for_future_teams(cls, team_ids: Iterable[int]) -> list[VpnStatus]:
        ...
        return [
            VpnStatus(
                ...
                self_active=service_status[
                    SystemCtlUtils.service_vpn_selfhosted(team_id)
                ],
                cloud_active=service_status[
                    SystemCtlUtils.service_vpn_cloudhosted(team_id)
                ],
                cloud_players_active=service_status[
                    SystemCtlUtils.service_vpn_cloud_players(team_id)
                ],
            )
            for team_id in team_ids
        ]
    ...
    def from_status(cls, status: VpnStatus) -> "VpnStatusCheck":
        ...
        if status.cloud_connected:
            ...
            if status.self_active:
                result.add_error(
                    "Self-hosted VPN is active although cloud-hosted VPN is connected."
                )
                result.fix_stop_service.append(
                    SystemCtlUtils.service_vpn_selfhosted(status.team_id)
                )
            if not status.cloud_active:
                result.add_error("Cloud-hosted VPN is connected but inactive (wtf?).")
                result.fix_start_service.append(
                    SystemCtlUtils.service_vpn_cloud_players(status.team_id)
                )
            ...
            if status.cloud_player_conns > 0:
                result.add_error(
                    "Both self-hosted and cloud player VPNs have connections."
                )
            ...
            if not status.cloud_active:
                ...
                result.fix_start_service.append(
                    SystemCtlUtils.service_vpn_cloudhosted(status.team_id)
                )
            if status.cloud_players_active:
                result.add_error(
                    "Cloud players VPN is active while self-hosted VPN is connected."
                )
                result.fix_stop_service.append(
                    SystemCtlUtils.service_vpn_cloud_players(status.team_id)
                )
...
class VpnStatusFixer:
    ...
    def reset(cls, check_list: list[VpnStatusCheck]) -> None:
        ...
        changes = Team.query.filter(Team.id.in_(ids)).update(
            {"vpn_connected": False, "vpn2_connected": False, "vpn_connection_count": 0}
        )
...
def command_status(args: argparse.Namespace) -> None:
    team_status = (
        VpnStatusFactory.from_team_ids(args.teamid)
        if args.teamid
        else VpnStatusFactory.for_all_teams()
    )
    errors = 0

    print(CliColors.HEADER + "-" * 55 + CliColors.ENDC)
    print(
        f"{CliColors.HEADER}Team      | Mode  | Self VPN  | Cloud VPN | Player VPN{CliColors.ENDC}"
    )
    print(CliColors.HEADER + "-" * 55 + CliColors.ENDC)
    for status in team_status:
        check = VpnStatusCheck.from_status(status)
        state_self = (
            f"{CliColors.OKGREEN}connected{CliColors.ENDC}"
            if status.self_connected
        ...
        state_players = (
            f"{CliColors.OKGREEN}{status.cloud_player_conns:>4} conns{CliColors.ENDC}"
            if status.cloud_player_conns > 0
            else ("on" if status.cloud_players_active else "off")
        )
        line = (
            f"Team #{status.team_id:3} | {check.mode or '-':5} | "
    ...
    if errors > 0:
        ...
        print(
            f"=> {len(team_status)} VPNs, {CliColors.OKGREEN}all good{CliColors.ENDC}"
        )
...
def command_set(args: argparse.Namespace) -> None:
    ...
    services_players = [
        SystemCtlUtils.service_vpn_cloud_players(i) for i in args.teamid
    ]
    if args.mode == "self":
...
def command_startup(args: argparse.Namespace) -> None:
    ...
    status_list += VpnStatusFactory.for_future_teams(
        set(team_ids) - set(s.team_id for s in status_list)
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--dry",
        action="store_true",
        help="Don't actually start/stop services or modify the DB.",
    )
    subparsers = parser.add_subparsers(dest="command", help="subcommands")

    parser_status = subparsers.add_parser("status", help="get VPN status")
    parser_status.add_argument(
        "teamid", nargs="*", type=int, help="Team IDs (optional)"
    )

    parser_fix = subparsers.add_parser(
        "fix", help="fix issues with the current VPN status"
    )
    ...
    parser_set = subparsers.add_parser(
        "set", help="manually choose cloud- or selfhosted (or off)"
    )
