...
def main():
    ...
    if is_vpn_2:
        changes = Team.query.filter(Team.id == team_id).update(
            dict(vpn2_connected=True, vpn_last_connect=func.now()),
            synchronize_session=False,
        )
    else:
        changes = Team.query.filter(Team.id == team_id).update(
            dict(vpn_connected=True, vpn_last_connect=func.now()),
            synchronize_session=False,
        )
    db_session().commit()
    if changes > 0:
        ...
        if is_vpn_2:
            session.add(
                Team(
                    id=team_id,
                    name=f"unnamed team #{team_id}",
                    vpn2_connected=True,
                    vpn_last_connect=func.now(),
                )
            )
        else:
            session.add(
        ...
        session.commit()
        print(
            f"Updated connection status (connected) of team #{team_id}. Created new dummy team entry for that."
        )
    else:
