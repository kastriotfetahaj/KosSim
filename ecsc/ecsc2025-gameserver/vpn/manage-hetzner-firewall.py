...
def update_state(state: str) -> None:
    ...
    if firewall.rules is None:
        raise ValueError(
            f"Firewall has no rules list ({firewall} / {repr(firewall.rules)}"
        )
    rule: FirewallRule
    ssh_rule: Optional[FirewallRule] = None
    for rule in firewall.rules:
        if (
            rule.protocol == FirewallRule.PROTOCOL_TCP
            and rule.direction == FirewallRule.DIRECTION_IN
            and rule.port == "22"
            and rule.source_ips == SOURCE_IPS
        ):
            ssh_rule = rule
    if allow_ssh == (ssh_rule is not None):
        logging.info(
            "[-]   No change necessary, SSH is "
            + ("allowed" if ssh_rule is not None else "blocked")
        )
...
def main() -> None:
    ...
    for item in pubsub.listen():
        if item["type"] == "message":
            logging.info(
                f"[debug] message {repr(item['channel'])}, data {repr(item['data'])}"
            )
