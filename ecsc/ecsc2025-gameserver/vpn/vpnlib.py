...
def generate_vpn_keys(directory: Path) -> None:
    ...
    subprocess.check_call(
        [EASYRSA_BINARY, "build-ca", "nopass"], env=env, cwd=directory
    )
    subprocess.check_call(
        [EASYRSA_BINARY, "gen-req", "server", "nopass"], env=env, cwd=directory
    )
    subprocess.check_call(
        [EASYRSA_BINARY, "gen-req", "TeamMember", "nopass"], env=env, cwd=directory
    )
    subprocess.check_call(
        [EASYRSA_BINARY, "sign-req", "server", "server"], env=env, cwd=directory
    )
    subprocess.check_call(
        [EASYRSA_BINARY, "sign-req", "client", "TeamMember"], env=env, cwd=directory
    )
...
def generate_additional_vpn_keys(directory: str, name: str) -> None:
    ...
    env["EASYRSA_BATCH"] = "1"
    subprocess.check_call(
        [EASYRSA_BINARY, "gen-req", name, "nopass"], env=env, cwd=directory
    )
    subprocess.check_call(
        [EASYRSA_BINARY, "sign-req", "client", name], env=env, cwd=directory
    )
