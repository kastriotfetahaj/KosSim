...
def build_systemd_file(teams: List[Team]):
    ...
    lines = [
        f"After=vpn2@team{team.id}-cloud.service\nRequires=vpn2@team{team.id}-cloud.service"
        for team in teams
    ]
    lines += [
        f"After=vpn@team{team.id}-vulnbox.service\nRequires=vpn@team{team.id}-vulnbox.service"
        for team in teams
    ]
...
def main():
    ...
    print(
        f'SETUP: rm -r /etc/openvpn/server && ln -s "{server_root}" /etc/openvpn/server\n\n'
    )
