...
def main():
    ...
    if is_vpn_2:
        changes = (
            ...
            .filter(Team.vpn2_connected == True)
            .update(
                dict(vpn2_connected=False, vpn_last_disconnect=func.now()),
        ...
        changes = (
            Team.query.filter(Team.id == team_id)
            .filter(Team.vpn_connected == True)
            .update(
                dict(vpn_connected=False, vpn_last_disconnect=func.now()),
                synchronize_session=False,
            )
        )
    db_session().commit()
