...
RULE_NETTEAMONLY = [
    ...
    "comment",
    "--comment",
    "Network is open for teams only",
]
...
def iptables_remove(chain: str, rule: List[str]) -> None:
    while (
        subprocess.call(["iptables", "-D", chain] + rule, stderr=subprocess.DEVNULL)
        == 0
    ):
        pass
...
class IpTablesManager:
    ...
    def init_firewall(
        self, state: str, banned: Iterable[str], allowed: Iterable[str]
    ) -> None:
    ...
    def rewrite_chain(self) -> None:
        with self._lock:
            if self.state == "on":
                ...
                logging.info(
                    "[TEAM] Network open within teams only, excluding vulnboxes"
                )
            else:
                iptables_flush(self.chain)
                iptables_add(self.chain, RULE_NETCLOSED)
    ...
    def remove_network_team(self, team: str) -> None:
        ...
        logging.info(
            f"[-A]   Remove network for team {team} ({iprange1} <-> {iprange2})"
        )
        ...
        while self._running:
            try:
                ...
                if num_teams != current_num_teams and self._running:
                    logging.info(
                        f"[team] Number of teams changed from {num_teams} to {current_num_teams}, re-writing rules..."
                    )
...
def manage_iptables(redis: StrictRedis) -> None:
    ...
    pubsub.subscribe(
        ...
        "network:ban",
        "network:unban",
        "network:add_permission",
        "network:remove_permission",
    )

    manager = IpTablesManager(CHAIN)
    manager.init_firewall(
        state, [team.decode() for team in banned], [team.decode() for team in allowed]
    )
        ...
        for item in pubsub.listen():
            if item["type"] == "message":
                if item["channel"] == b"network:state":
                    ...
                    logging.info(
                        f"[debug] unknown message {repr(item['channel'])}, data {repr(item['data'])}"
                    )
