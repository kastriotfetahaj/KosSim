...
class ServiceWithTimeout(Service):
    ...
    def __init__(
        ...
        wrapper = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "selenium_wrapper"
        )
...
class SeleniumServiceInterface(ServiceInterface, ABC):
    ...
    blocked_urls: list[
        str
    ] = []  # a list of url patterns that should not be accessed (example: "*/bootstrap.min.css")
    ...
    def get_selenium_webdriver(
        self, arguments: list[str] | None = None, timeout: int = TIMEOUT
    ) -> WebDriver:
        if self.webdriver:
        ...
        self.webdriver = webdriver.Remote(
            self.service.service_url, options=options, desired_capabilities=caps
        )
        self.webdriver.set_page_load_timeout(timeout)
        # add missing support for chrome "send_command"  to selenium webdriver
        self.webdriver.command_executor._commands["send_command"] = (
            "POST",
    ...
    def wait_for_element_by_id(self, driver: WebDriver, id: str) -> None:
        ...
        try:
            ...
            raise OfflineException(
                f"Timed out waiting for page to load / element '{id}' to appear"
            )
    ...
    def wait_for_element_by_xpath(self, driver: WebDriver, xpath: str) -> None:
        ...
        try:
            ...
            raise OfflineException(
                "Timed out waiting for page to load / link to appear"
            )
