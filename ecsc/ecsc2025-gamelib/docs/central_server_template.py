...
if not CONFIG:
    ...
    CONFIG = {
        "databases": {"redis": {"host": "localhost", "port": 6379, "db": 0}},
        "network": {"team_range": [10, [200, 256, 32], [1, 200, 0], 0, 24]},
    }
network_ip: List[Tuple[int, int, int]] = [
    tuple(component) if type(component) is list else (1, 1, component)
    for component in CONFIG["network"]["team_range"][:4]
]  # type: ignore
...
def validate_service_token(team_id: int, token: str) -> bool:
    ...
    """
    expected = hmac.HMAC(
        SERVICE_TOKEN_SECRET.encode(), str(team_id).encode(), hashlib.sha256
    ).hexdigest()[:32]
...
class GameStatus:
    ...
    def __init__(self) -> None:
        ...
        thread = threading.Thread(
            target=self.__listen_for_redis_events, name="Redis-Listener", daemon=True
        )
        thread.start()
